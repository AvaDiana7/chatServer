...
public class ChatGptServiceImpl implements ChatGptService {
    ...
    public Message getChat(List<Message> messages) {
        ...
        GptData data = new GptData(messages, "gpt-3.5-turbo", 1000, 0.8f, 1, 1, false);
        // 请求体搭建
    ...
    @Override
    synchronized public InputStream getChatStream(List<Message> messages, boolean vip) {
        String key = "";
        key = vip ? apikey : apikey2[apikeyIndex++ % apikey2.length];


        GptData data = new GptData(messages, "gpt-3.5-turbo", 1000, 0.8f, 1, 1, true);
        // 请求体搭建
        RequestBody requestBody = RequestBody.create(JSON.toJSONString(data), MediaType.parse("application/json"));
        // 请求搭建
        Request request = new Request.Builder()
